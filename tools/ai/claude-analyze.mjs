...
function generateDocsMarkdown(components, navItems, pages, restRoutes) {
  ...
  md.push('');
  components.forEach(comp => {
    md.push(`- [ ] **${comp.name}** (${comp.active ? 'Active' : 'Inactive'})`);
...
function generatePHPUnitFiles(components, restRoutes) {
  ...
  components.filter(c => c.active).forEach(comp => {
    const className = comp.name.replace(/[^a-zA-Z0-9]/g, '') + 'IntegrationTest';
    const code = `<?php
/**
 * Integration tests for ${comp.name} component
 * 
 * @package ${slug}
 */
...
class ${className} extends WP_UnitTestCase {
    ...
    public function test_component_is_active() {
        $this->assertTrue(
            ...
            '${comp.name} component should be active'
        );
    }
    ...
    public function test_component_initialization() {
        ...
        $this->assertNotNull(
            buddypress()->${comp.id},
            '${comp.name} component should be initialized'
        );
    }
